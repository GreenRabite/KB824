document.addEventListener('DOMContentLoaded', ()=> {
  ...
  d3.csv("./data/kobe-reg-games-log.csv", type, function(error, data) {
    ...
    data.forEach((d)=>{
      ...
      homeAwaySplits["homeAvg"] = homeAwaySplits["home"] / homeAwaySplits["homeGames"];
      homeAwaySplits["roadAvg"] = homeAwaySplits["road"] / homeAwaySplits["roadGames"];
      // Team Splits
        ...
        .attr("fill", "#552583")
          ...
          .on("mouseover", function(d) {
            tooltip.transition()
                .duration(200)
                .style("opacity", 1);
        tooltip.html(`Kobe scored ${d.PTS} points against the ${d.Opp} on ${d.Date}.`)
             .style("left", (d3.event.pageX + 5) + "px")
             .style("top", (d3.event.pageY - 28) + "px");
    })
        .on("mouseout", function(d) {
...
d3.select(".blks") //Blocks
    ...
    //.ease("linear")  // Transition easing - default 'variable' (i.e. has acceleration), also: 'circle', 'elastic', 'bounce', 'linear'
            ...
            d3.select(this)  // 'this' means the current element
                ...
                .attr("fill", "#552583")
                // .attr("fill", "black")  // Change color
                .attr("r", (d) => { return (d.BLK / .4) });  // Change radius
        });
...
d3.select(".stls") //Steals
    ...
    //.ease("linear")  // Transition easing - default 'variable' (i.e. has acceleration), also: 'circle', 'elastic', 'bounce', 'linear'
            ...
            d3.select(this)  // 'this' means the current element
                ...
                .attr("fill", "#552583")
                // .attr("fill", "black")  // Change color
                .attr("r", (d) => { return (d.STL / .6) });  // Change radius
        });
...
d3.select(".gmsc") // Game Score
    ...
    //.ease("linear")  // Transition easing - default 'variable' (i.e. has acceleration), also: 'circle', 'elastic', 'bounce', 'linear'
            ...
            d3.select(this)  // 'this' means the current element
                ...
                .attr("fill", "#552583")
                // .attr("fill", "black")  // Change color
                .attr("r", (d) => { return (d.GmSc / 5) });  // Change radius
        });
...
d3.select(".tovs") //Turnovers
    ...
    //.ease("linear")  // Transition easing - default 'variable' (i.e. has acceleration), also: 'circle', 'elastic', 'bounce', 'linear'
            ...
            d3.select(this)  // 'this' means the current element
                ...
                .attr("fill", "#552583")
                // .attr("fill", "black")  // Change color
                .attr("r", (d) => { return (d.TOV) });  // Change radius
        });
